 ...
 * Styled for Responsive System / Light / Dark Themes.
 */
...
export const AlcoNavigator: React.FC<AlcoNavigatorProps> = ({
  ...
  return (
    <section
      ...
      className="bg-white dark:bg-slate-900/90 rounded-2xl border border-slate-200 dark:border-slate-800 shadow-sm dark:shadow-xl overflow-hidden"
    >
      {/* Header bar */}
      <div className="bg-slate-50 dark:bg-slate-950 p-5 sm:p-6 border-b border-slate-200 dark:border-slate-800 flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div className="flex items-center gap-3">
          <div className="w-10 h-10 rounded-xl bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/30 text-indigo-600 dark:text-indigo-400 flex items-center justify-center shrink-0">
            <Sparkles className="w-5 h-5" aria-hidden="true" />
          </div>
          <div>
            <div className="flex items-center gap-2">
              <h2 id="heading-alco-navigator" className="text-base sm:text-lg font-bold tracking-tight text-slate-900 dark:text-white">
                Tanya ALCO Navigator
              </h2>
              <span className="px-2 py-0.5 rounded text-[10px] font-semibold bg-indigo-50 dark:bg-indigo-500/10 text-indigo-700 dark:text-indigo-300 border border-indigo-200 dark:border-indigo-500/20 uppercase tracking-wider">
                AI Asisten
              </span>
            </div>
        ...
        <div className="flex items-center gap-2">
          {apiKey ? (
            <div className="flex items-center gap-2 text-xs">
              <span className="inline-flex items-center gap-1.5 px-3 py-1.5 rounded-full bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20">
              ...
              <button
                ...
                className="text-xs text-slate-500 dark:text-slate-400 hover:text-slate-900 dark:hover:text-white underline underline-offset-2"
              >
                Ganti Key
              </button>
            </div>
      ...
      {/* Navigation Sub-Tabs */}
      <div className="px-5 sm:px-6 pt-4 border-b border-slate-200 dark:border-slate-800 bg-slate-50/50 dark:bg-slate-900/60 flex items-center gap-2">
        <button
          type="button"
          onClick={() => setActiveTab('consult')}
          className={`pb-3 px-2 text-xs font-bold transition-all border-b-2 inline-flex items-center gap-2 ${
            activeTab === 'consult'
              ? 'border-indigo-600 dark:border-indigo-400 text-indigo-600 dark:text-white'
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-2">
                {QUICK_CHOICES.map((choice) => (
                  <button
                    ...
                    className="px-3 py-1.5 rounded-lg text-xs font-medium bg-slate-100 hover:bg-slate-200 dark:bg-slate-800 dark:hover:bg-slate-700 text-slate-800 dark:text-slate-200 border border-slate-200 dark:border-slate-700 transition-colors disabled:opacity-50 text-left"
                  >
                    {choice}
                  </button>
                ))}
            ...
            >
              <input
                ...
                className="flex-1 px-4 py-2.5 text-xs bg-slate-50 dark:bg-slate-950 border border-slate-300 dark:border-slate-800 rounded-lg text-slate-900 dark:text-white placeholder:text-slate-400 dark:placeholder:text-slate-500 focus:outline-none focus:border-indigo-500 focus:ring-1 focus:ring-indigo-500"
              />
              <button
                type="submit"
            ...
            {/* Error Message */}
            {adviceError && (
              <div className="p-4 rounded-xl bg-rose-50 dark:bg-rose-500/10 border border-rose-200 dark:border-rose-500/20 text-rose-800 dark:text-rose-300 text-xs flex items-start gap-2.5">
                <AlertCircle className="w-4 h-4 text-rose-600 dark:text-rose-400 shrink-0 mt-0.5" />
                <div>
                  <p className="font-semibold">Gagal memproses rekomendasi</p>
                  <p className="text-rose-700 dark:text-rose-300/80 mt-0.5">{adviceError}</p>
                </div>
              </div>
            )}
            ...
            {/* Result Display */}
            {adviceResult && (
              <div className="p-5 rounded-xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 space-y-4 animate-in fade-in">
                ...
                <div className="p-4 rounded-lg bg-indigo-50 dark:bg-indigo-500/10 border border-indigo-200 dark:border-indigo-500/20 space-y-2">
                  <div className="flex items-center justify-between gap-2">
                    ...
                    <span className="px-2 py-0.5 rounded text-[10px] font-bold bg-indigo-100 dark:bg-indigo-500/20 text-indigo-800 dark:text-indigo-200">
                      ...
                      {(() => {
                        ...
                        return (
                          <button
                            ...
                            className="inline-flex items-center gap-2 px-3.5 py-1.5 rounded-md bg-indigo-600 dark:bg-indigo-500 hover:bg-indigo-500 dark:hover:bg-indigo-400 text-white dark:text-slate-950 text-xs font-bold transition-all shadow-xs"
                          >
          ...
          <div className="space-y-6">
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                {PROJECT_CONDITIONS.map((cond) => (
                  <button
                    ...
                    className={`p-3.5 rounded-xl border text-left transition-all ${
                      selectedCondition === cond.label
                        ? 'bg-indigo-50 dark:bg-indigo-500/10 border-indigo-300 dark:border-indigo-500/40 text-slate-900 dark:text-white'
                  ...
                  >
                    <p className="text-xs font-bold text-slate-900 dark:text-white">{cond.label}</p>
                    <p className="text-[11px] text-slate-500 dark:text-slate-400 mt-0.5">{cond.desc}</p>
                  </button>
                ))}
              </div>
            </div>
            ...
            {checkerResult && (
              <div className="p-5 rounded-xl bg-slate-50 dark:bg-slate-950/70 border border-slate-200 dark:border-slate-800 space-y-4 animate-in fade-in">
                ...
                <div className="space-y-2.5">
                  ...
                  {checkerResult.steps.map((step) => {
                    ...
                    return (
                      <div
                        key={step.stepNumber}
                        className="p-3.5 rounded-lg bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 flex items-start gap-3 text-xs"
                      >
                        <span className="w-5 h-5 rounded-full bg-indigo-100 dark:bg-indigo-500/20 text-indigo-700 dark:text-indigo-300 font-bold text-[10px] flex items-center justify-center shrink-0 mt-0.5">
                          {step.stepNumber}
                        </span>
                        <div className="flex-1 min-w-0">
                          <p className="font-bold text-slate-900 dark:text-white">{step.title}</p>
                          <p className="text-slate-600 dark:text-slate-400 text-[11px] mt-0.5 leading-relaxed">{step.description}</p>
                          {targetApp && (
                            <button
                              type="button"
                              onClick={() => onOpenAppUrl(targetApp)}
                              className="mt-2 inline-flex items-center gap-1.5 text-[11px] font-bold text-indigo-600 dark:text-indigo-400 hover:text-indigo-700 dark:hover:text-indigo-300"
                            >
                ...
                <div className="p-3 rounded-lg bg-slate-100/80 dark:bg-slate-900/60 border border-slate-200 dark:border-slate-800/80 text-xs text-slate-600 dark:text-slate-400">
                  <strong className="text-slate-800 dark:text-slate-200">Fokus Utama:</strong> {checkerResult.mainRecommendation}
                </div>
              </div>
            )}
          </div>
