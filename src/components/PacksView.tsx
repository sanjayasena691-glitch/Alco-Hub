      ...
      <div className="space-y-4">
        ...
        {activePacks.length > 0 ? (
          ...
          <div className="p-8 rounded-2xl bg-slate-900/40 border border-slate-800 text-center space-y-2">
            <p className="text-sm font-bold text-slate-300">Belum Ada Active Product Pack</p>
      ...
      <div className="space-y-4 pt-4 border-t border-slate-800">
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          {comingSoonPacks.map((pack) => {
            ...
            return (
              <div
                ...
                className="p-5 rounded-2xl bg-slate-900/70 border border-amber-500/20 hover:border-amber-500/40 space-y-3 transition-all flex flex-col justify-between"
              >
                <div className="space-y-2">
                  ...
                  <h4 className="text-base font-bold text-white tracking-tight">{pack.name}</h4>
                  {pack.tagline && (
                    <p className="text-xs font-medium text-slate-300">{pack.tagline}</p>
                  )}
                  <p className="text-xs text-slate-400 leading-relaxed line-clamp-3">{pack.description}</p>
                ...
                <div className="pt-3 border-t border-slate-800/80">
                  <button
                    type="button"
                    onClick={() => setSelectedPackId(pack.id)}
                    className="w-full inline-flex items-center justify-center gap-1.5 px-3 py-2 rounded-lg bg-slate-800/80 hover:bg-slate-700 text-slate-200 text-xs font-semibold transition-all"
                  >
          ...
          {uncreatedFutureCategories.map((fp) => (
            <div
              key={fp.id}
              className="p-5 rounded-2xl bg-slate-900/40 border border-slate-800/60 space-y-2 opacity-75 flex flex-col justify-between"
            >
              <div className="space-y-2">
                <div className="flex items-center justify-between">
                ...
                <h4 className="text-base font-bold text-slate-300">{fp.name}</h4>
