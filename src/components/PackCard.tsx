...
export const PackCard: React.FC<PackCardProps> = ({
  ...
  return (
    <div
      ...
      className="relative rounded-2xl bg-white dark:bg-gradient-to-br dark:from-slate-900/95 dark:via-slate-900/90 dark:to-slate-950 border border-slate-200 dark:border-slate-800 hover:border-slate-300 dark:hover:border-slate-700/90 p-6 sm:p-7 shadow-sm dark:shadow-xl transition-all duration-200 flex flex-col justify-between overflow-hidden group"
    >
      {/* Decorative accent background blur */}
      <div
        className={`absolute -right-16 -top-16 w-48 h-48 rounded-full blur-3xl pointer-events-none opacity-10 dark:opacity-15 transition-opacity group-hover:opacity-20 dark:group-hover:opacity-25 ${
          pack.accent === 'emerald'
            ? 'bg-emerald-500'
            : pack.accent === 'purple'
              ? 'bg-purple-500'
      ...
      <div className="relative z-10 space-y-5">
        ...
        <div className="flex items-center justify-between gap-3">
          ...
          <span
            ...
            className="inline-flex items-center px-2.5 py-1 rounded-full text-[11px] font-semibold bg-emerald-50 dark:bg-emerald-500/10 text-emerald-700 dark:text-emerald-300 border border-emerald-200 dark:border-emerald-500/20"
          >
              ...
              return (
                <div
                  ...
                  className={`px-3 py-2.5 rounded-lg border text-xs flex items-center justify-between gap-2 transition-colors ${
                    isComingSoon
                      ? 'bg-slate-50 dark:bg-slate-950/60 border-slate-200 dark:border-slate-800/60 text-slate-400 dark:text-slate-500'
                  ...
                  <div className="flex items-center gap-2 min-w-0">
                    <span
                      className={`w-2 h-2 rounded-full shrink-0 ${
                        isComingSoon ? 'bg-slate-400 dark:bg-slate-600' : 'bg-emerald-500 dark:bg-emerald-400'
                      }`}
                    />
      ...
      <div className="relative z-10 pt-6 mt-6 border-t border-slate-100 dark:border-slate-800/80 flex items-center justify-between">
        ...
        <button
          ...
          className="inline-flex items-center gap-2 px-4 py-2.5 rounded-lg bg-emerald-600 dark:bg-emerald-500 hover:bg-emerald-500 dark:hover:bg-emerald-400 text-white dark:text-slate-950 text-xs font-bold tracking-tight transition-all shadow-sm active:scale-[0.99] focus:outline-none focus:ring-2 focus:ring-emerald-400 focus:ring-offset-2 dark:focus:ring-offset-slate-950"
        >
